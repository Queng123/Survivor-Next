...
import { Alert, Text, View, StyleSheet, ScrollView, Pressable, FlatList } from 'react-native';
...
const YoutubeFavorites = ({token}: {token: string}): JSX.Element => {
    ...
    React.useEffect(() => {
      fetch('https://youtube.googleapis.com/youtube/v3/videos?part=snippet&myRating=like&mine=true&maxResults=50', {
        headers: {
      ...
      <View style={styles.favoriteView}>
        ...
        <ScrollView>
          {videos && videos.map((video, index) => (
            <Pressable key={`youtube-favorite-${index}`} onPress={() => Linking.openURL(`https://www.youtube.com/watch?v=${video.id}`)}>
              <View style={styles.favoriteWrapper}>
                ...
                <View style={styles.favoriteDesc}>
                  ...
                  <View style={{height: 6}} />
                  <Text style={{color: 'white', fontSize: 12}}>{video.snippet.channelTitle}</Text>
                  <Text style={{color: 'white', fontSize: 12}}>{youtubeDateToReadableDate(video.snippet.publishedAt)}</Text>
                </View>
              </View>
            </Pressable>
          ))}
